...
export function useListings(filters: ListingFilters = {}) {
  ...
  const fetchListings = useCallback(async () => {
    ...
    setLoading(false);
  }, [filters.search, filters.minPrice, filters.maxPrice, filters.bedrooms, filters.isFurnished, filters.utilitiesIncluded]);
...
export function useSavedListings(userId: string | undefined) {
  ...
  useEffect(() => {
    if (!userId) return;
    supabase
      .from('saved_listings').select('listing_id').eq('user_id', userId)
      .then(({ data }) => {
