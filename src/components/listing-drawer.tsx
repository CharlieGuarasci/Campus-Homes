...
export function ListingDrawer({ listing, open, onClose, currentUserId }: ListingDrawerProps) {
  ...
  useEffect(() => {
    ...
    async function fetchRelated() {
      ...
      if (data) {
        setRelated(data as ListingWithDetails[]);
      }
    }
    fetchRelated();
  }, [listing, supabase]);
    ...
    <Drawer.Root open={open} onOpenChange={(v) => !v && onClose()}>
      <Drawer.Portal>
        ...
        <Drawer.Content className="fixed bottom-0 left-0 right-0 z-50 flex flex-col bg-white rounded-t-2xl max-h-[92vh] focus:outline-none">
          ...
          <button
            onClick={onClose}
            className="absolute top-4 right-4 z-10 h-8 w-8 rounded-full bg-white/90 flex items-center justify-center shadow-sm"
          >
